...
#
# Total is the sum of each slot's own Capacity (8GB + 8GB = 16GB), not
# Win32_ComputerSystem.TotalPhysicalMemory: that one reports usable memory
# minus whatever's reserved for hardware, which can land well under X.5
# (e.g. 15.2 for a 16GB kit) and would round DOWN to the wrong whole GB.
# Per-slot Capacity is the stick's real nominal size, always exact - only
# fall back to TotalPhysicalMemory when there is no slot data at all.
